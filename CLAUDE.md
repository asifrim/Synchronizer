...
- **Event editing interactions:** left-click selects an event (amber outline); `d` toggles it deleted/restored (disabled events are excluded from MIDI and from Ctrl+S saves); Esc deselects. Left-drag horizontally shifts the event's `start_time` (persisted on Ctrl+S); left-drag vertically reassigns its cluster within the active k. Shift+left-click previews the event in isolation. Right-drag changes a bucket value up/down. Digit keys 0–9 reassign the hovered event's cluster.
...
The CLI saves two additional sidecars after every run: `embeddings.npy` (the raw `(n_transients, 2048)` matrix, for offline analysis and plotting) and `umap_clusters.png` (a 2×4 grid of 2-D UMAP scatter plots, one panel per k=2..8 plus the best-k result, produced by `plot.write_umap_plots`).
